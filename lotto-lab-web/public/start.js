...
async function loadResult() {
  ...
  try {
    ...
    if (!data.ok || latest?.game !== 'tw539' || !/^\d{4}-\d{2}-\d{2}$/.test(latest.date) || !latest.period || !Array.isArray(latest.numbers) || latest.numbers.length !== 5 || new Set(latest.numbers).size !== 5 || !latest.numbers.every(n=>Number.isInteger(n)&&n>=1&&n<=39)) throw new Error('invalid');
