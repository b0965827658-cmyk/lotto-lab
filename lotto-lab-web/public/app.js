...
async function getServiceWorkerRegistration() {
  ...
  if (!state.serviceWorkerRegistration) {
    state.serviceWorkerRegistration = await navigator.serviceWorker.register("/sw.js?v=83", {
      updateViaCache: "none",
    });
  }
  try {
...
async function showLocalTestNotification(title = "摘星引擎開獎通知", body = "這是一則測試通知。", options = {}) {
  if (!notificationSupported()) {
    setStatus("這個瀏覽器目前不支援通知。", true);
    return;
  }
...
async function notifyIfLatestChanged(latest, previousKey) {
  ...
  const title = `${latest.name || "摘星引擎"} 已更新`;
...
if (els.notifyTest) {
  els.notifyTest.addEventListener("click", () => {
    ...
    const title = latest ? `${latest.name} 最新開獎通知` : "摘星引擎開獎通知";
