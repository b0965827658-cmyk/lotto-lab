...
GAMES: tuple[LotteryGame, ...] = (
    LotteryGame("tw539", "今彩539", "台灣", 5, 1, 39, "已連線，資料驗證中", "official-adapter-pending"),
    LotteryGame("ca-fantasy5", "加州天天樂", "美國加州", 5, 1, 39, "已連線，資料驗證中", "official-adapter-pending"),
    LotteryGame("mark-six", "六合彩", "香港", 6, 1, 49, "資料來源驗證中", "not-started"),
    LotteryGame("power-lottery", "威力彩", "台灣", 6, 1, 38, "資料來源驗證中", "not-started"),
    LotteryGame("lotto-649", "大樂透", "台灣", 6, 1, 49, "資料來源驗證中", "not-started"),
    LotteryGame("daily-3", "三星彩", "台灣", 3, 0, 9, "資料來源驗證中", "not-started"),
    LotteryGame("daily-4", "四星彩", "台灣", 4, 0, 9, "資料來源驗證中", "not-started"),
)
...
def validate_main_numbers(game_code: str, numbers: list[int]) -> bool:
    ...
    return (
        ...
        and len(set(numbers)) == game.draw_size
